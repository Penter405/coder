...
class ChangeApplier {
    ...
    */
    parseToInstructions(aiResponse, rootPath) {
        const instructions = [];
        ...
        const pushInst = (inst, startLineRel, endLineRel) => {
            instructions.push({
                id: idCounter++,
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (state === 'ROOT') {
                ...
                if (mkDirMatch) {
                    ...
                    state = 'OP_BLOCK';
                    opAction = null; // Mark opAction null to avoid duplicated if logic handled here
    ...
    applyInstructions(instructions) {
        ...
        for (const inst of instructions) {
            ...
            if (!fileOps.has(targetPath))
                fileOps.set(targetPath, []);
        ...
        for (const [originalFilePath, ops] of fileOps) {
            ...
            modOps.sort((a, b) => {
                const getLine = (op) => {
                    if (op.action === 'ADD')
                        return op.line || 0;
                    if (op.action === 'ADD_AFTER')
                        return (op.line || 0) + 1; // Effective insertion index
                    if (op.action === 'REMOVE')
                        return op.start || 0;
                    return 0;
                };
